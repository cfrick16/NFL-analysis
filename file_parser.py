...
def parseFiles(years):
	for year in years:
		...
		for line in lines[1:]:
			yield dict(zip(key, line.split(",")))

# Get list of games given years and optional columns
def getGames(years, relevent_columns=categories):
	if("game_id" not in relevent_columns):
		relevent_columns = ["game_id"] + relevent_columns
	game = []
	last_game_id = -1
	for plays in parseFiles(years):
		d = dict((k, plays[k]) for k in relevent_columns)
		if(last_game_id != d["game_id"]):
			if(len(game) != 0):
				yield game
			
			last_game_id = d["game_id"]
			game = []
